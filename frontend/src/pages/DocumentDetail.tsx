...
export function DocumentDetail() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    refetchInterval: (query) => {
      const d = query.state.data as Detail | undefined;
  ...
  return (
    <div className="flex h-[calc(100vh-49px)] flex-col">
      ...
      {isLoading && <p className="p-8 text-neutral-400">Carregando…</p>}
                            ...
                            {editable ? (
                              <>
                                ...
                                <td className="py-1 pl-1 text-center">
                                  <button onClick={() => removeLine(i, li)} className="text-neutral-300 hover:text-red-600" title="Remover linha">✕</button>
                                </td>
                              </>
                            ) : (
                              <>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              {editable ? (
                <>
                  ...
                  >
                    {reject.isPending ? "…" : "Rejeitar"}
                  </button>
                  {dirty && !busy && <span className="text-xs text-amber-600">Há alterações não salvas</span>}
                </>
              ) : data.status === "approved" ? (
                <span className="inline-flex items-center gap-2 text-sm font-medium text-emerald-700">
                  ✓ Documento aprovado (somente leitura)
                </span>
